...
#include "localpicturesthumbnailprovider.h"

#include <KIO/PreviewJob>

class PicturesThumbnailProvider::Private
{
public:
    Private() {
    }
...
PicturesThumbnailProvider::PicturesThumbnailProvider(QObject* parent)
    : QObject(parent)
...
PicturesThumbnailProvider::~PicturesThumbnailProvider()
{
    delete d;
}

void PicturesThumbnailProvider::loadThumbnails(const KUrl::List& fileList, const QSize& size)
{
...
void PicturesThumbnailProvider::loadThumbnail(const KUrl& file, const QSize& size)
{
    loadThumbnails(KUrl::List() << file, size);
}

void PicturesThumbnailProvider::processPreview(const KFileItem& file, const QPixmap& thumbnail)
{
    d->pixmapCache.insert(file.url().prettyUrl(), thumbnail);
    emit gotThumbnail(file.url().prettyUrl());
}

QPixmap PicturesThumbnailProvider::requestPixmap(const QString& id, QSize* size, const QSize& requestedSize)
{
...
bool PicturesThumbnailProvider::hasThumbnail(const QString& url)
{
